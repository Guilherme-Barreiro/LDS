...
namespace ConsultaPlus.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordDto dto)
        {
            try
            {
                // Vamos assumir que por agora só funciona para pacientes
                // Mais tarde tornaremos isto mais genérico
                await _authService.ForgotPasswordAsync(dto.Email);

                // Retornamos sempre OK para não dar pistas a atacantes
                return Ok(new { message = "Se a sua conta existir, um email foi enviado com as instruções para redefinir a sua password." });
            }
            catch (Exception ex)
            {
                // Numa aplicação real, devemos fazer log do erro mas nunca expô-lo
