...
namespace ConsultaPlus.Infrastructure.Services // (ou Application.Services, se tiveres o projeto)
{
    ...
    public class EspecialidadesService : IEspecialidadesService
    {
        ...
        {
            var e = await _repo.GetByIdAsync(id) ?? throw new KeyNotFoundException("Especialidade não existe.");
            var n = (novoNome ?? "").Trim();
            if (string.IsNullOrWhiteSpace(n))
                throw new ArgumentException("Nome é obrigatório.");
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            if (await _repo.HasMedicosAsync(id))
                throw new InvalidOperationException("Não é possível remover: existem médicos associados.");
